...
mas* create_mas(int capasity, char name)
{
    ...
    res->array = malloc(sizeof(int) * capasity);
...
{//A, b;
    if (!isalpha(str[0])) return INVALID;
    if (str[1] != ',' || str[2] != ' ') return INVALID;
    if (!isalpha(str[3])) return INVALID;
    if (strlen(str) != 4) return INVALID;
    char name_mas = toupper(str[0]);
...
{//a, 2, 7
    if (!isalpha(str[0])) return INVALID;
    if (str[1] != ',' || str[2] != ' ') return INVALID;
    if (!isdigit(str[3])) return INVALID;
    char name_mas = toupper(str[0]);
    ...
    num2[index_num_2] = '\0';
    if (!checkInt(num1) || !checkInt(num2)) return INVALID;
    int num_index = atoi(num1); //2
    int num_count = atoi(num2);// 3;
    if (num_index > arr->size || num_index < 0 || num_count < 0 || num_index + num_count > arr->size) return ERROR_MEMORY;
...
{
    if (!isalpha(str[0]) || str[1] != ',' || str[2] != ' ' || !isdigit(str[3])) return INVALID;
    char name_mas = toupper(str[0]);
    ...
    if (strlen(first_num) == 0 || strlen(second_num) == 0) return INVALID;
    i += 2;
    if (!isalpha(str_after_name[i])) return INVALID;
    char name_mas2 = toupper(str_after_name[i]);
    mas* arr2 = all_mass[(int)name_mas2 - 65];
    int index1 = atoi(first_num);
    int index_2 = atoi(second_num);
    if (index1 > index_2 || index_2 >= arr1->size || index1 < 0 ) return ERROR_MEMORY;
...
{
    if (strlen(str) > 2 || strlen(str) < 2) return INVALID;
    if (!isalpha(str[0])) return INVALID;
    if (str[1] != '+' && str[1] != '-') return INVALID;
    char name_mas = toupper(str[0]);
    mas* arr = all_mass[(int)name_mas - 65];
    if (str[1] == '+')
    {
    ...
    else qsort(arr->array, arr->size, sizeof(int), compareDown);
    return OK;   
}
enum checker fShuffle(char* str, mas** all_mass)
{
    if (strlen(str) != 1 || !isalpha(str[0])) return INVALID;
...
double find_max_away(int* array, int size, double middle_value)
{
    ...
    {
        if (abs(array[i] - middle_value) > max_away) max_away = array[i] - middle_value;
    }
    return max_away;
}
...
{//a, 3
    ...
    if (str_after_print[i] != ',')
    {
        ...
        if (index < 0 || index > arr->size) return ERROR_MEMORY;
        printf("%d\n", arr->array[index]);
        return OK;
    }
    else if (str_after_print[i] == ',')
    {
        ...
        int index_2 = atoi(second_num);
        if (index_2 < index_1 || index_2 > arr->size || index_1 < 0) return ERROR_MEMORY;
...
{
    if (strlen(str) != 2 || !isalpha(str[0])) return INVALID;
    mas* arr = all_mass[(int)(toupper(str[0])) - 65];
    if (arr->array != NULL) free(arr->array);
    arr->array = NULL;
    arr->capasity = 0;
    arr->size = 0;
    return OK;
...
void free_all(mas** all_mass)
{
    for (int i = 0; i < 26; i++)
    {
        ...
        if (all_mass[i] != NULL) free(all_mass[i]);
    }
}
