    ...
    {  //Флаг S
        char *res;
        int flag;
        flagS(absNum, &res, &flag);

        if(flag==-1) 
        {
            printf("Недостаточно памяти или введен 0.\n");
            return 0;
        }

        if(res[flag]==0) flag-=1;
    ...
    if(argv[2][1]=='f')
    {
        ...
        (fact(absNum)==-1) ? printf("Вы ввели слишком большое число, результат не помещается в int.\n") : printf("Факториал %d = %d\n", absNum, fact(absNum));
        return 0;
    }
