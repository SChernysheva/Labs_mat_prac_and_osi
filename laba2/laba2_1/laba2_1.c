...
enum Flag_str checkInt(char* str){
    int len =FlagL(str);
    if (len >= 10 || len <= 0){
        return ZERO_STR;
    }
    ...
    *res = (char*)malloc(sizeof(char) * len);
    ...
    for (int i = 0; i <= len; i++){
        if (isdigit(str[i])){
            ...
            if (digits[d] == '\0'){
                temp = realloc(digits, sizeof(char) * (d + 1) * 2);
                if (!temp){
                    freeForFlagN(digits, alphas, others);
                    return ERROR_MEMORY;
                }
            ...
            if (alphas[a] == '\0'){
                temp = realloc(alphas, sizeof(char) * (a + 1) * 2);
                if (!temp){
                    freeForFlagN(digits, alphas, others);
                    return ERROR_MEMORY;
                }
            ...
            if (others[o] == '\0'){
                temp = realloc(others, sizeof(char) * (o + 1) * 2);
                if (!temp){
                    freeForFlagN(digits, alphas, others);
                    return ERROR_MEMORY;
                }
...
int main(int argc, char*argv[])
{
    ...
    if (argv[1][1] == 'c'){
        ...
        enum Flag_str ans = checkInt(argv[2]);
        if (ans != OK){
            printf("Невалидный аргумент для флага с\n");
            return 1;
        }
